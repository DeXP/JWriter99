...
#define JSTDCALL_ATTR __attribute__((stdcall))
...
struct TJsonWrStruct {
    ...
    int (JSTDCALL_ATTR* jsonwr_vsprintf) (char* s, const char* format, va_list arg);
};
...
#ifndef JWR99_NO_STDLIB
#include <stdio.h>
#include <stdlib.h>
int console_putc(int character, void* userData) {
    (void)userData; return putchar(character);
}

#define JStartConsole           JStart(0, console_putc, vsprintf)
#define JStartConsoleObj        JStartConsole JObjBlock
#define JStartConsoleArr        JStartConsole JArrBlock

// Static memory macroses
#define JStartStaticMemEx(JsonBuffer, MaxSize, isFlat)    JStart(isFlat, stat_mem_putc, (int (JSTDCALL_ATTR*)(char *, const char *, va_list)) vsprintf)JStaticMemOpen(JsonBuffer, MaxSize)
...
#define JStartMemEx(JsonBuffer, isFlat)    JStart(isFlat, dyn_mem_putc, (int (JSTDCALL_ATTR*)(char *, const char *, va_list)) vsprintf)JMemOpen(JsonBuffer)
...
int file_putc(int character, void* userData) {
    return fputc(character, (FILE *)userData);
}

int file_close(void* userData) {
    if (NULL != userData) {
...
#define JFileOpen(fileName) \
       for (int _break = ((_JWrInternal.userData = fopen(fileName, "w+")), 1); _break; \
         _break = 0,file_close(_JWrInternal.userData)) \
            if (NULL != _JWrInternal.userData)

// File-based macro defines
#define JStartFileEx(fileName, isFlat)    JStart(isFlat, file_putc, vsprintf)JFileOpen(fileName)
...
#endif // JWR99_NO_STDLIB


#ifdef JWR99_WINAPI
#include <windows.h>

int WJPuts(const char* str) {
...
void* winapi_realloc(void* ptr, unsigned int size) {
    if (0 == ptr) return GlobalAlloc(GMEM_MOVEABLE, (SIZE_T)size);
        else return GlobalReAlloc((HGLOBAL)ptr, (SIZE_T)size, GMEM_MOVEABLE);
}

#define WJMemOpen(JsonBuffer) \
        for (struct TJsonMemBuffer _JMemBufInternal = { ._break = 1, .pos = 0, .size = 0, .data = NULL, .jsonwr_realloc = winapi_realloc}; \
        _JMemBufInternal._break; \
        _JMemBufInternal._break=0, JsonBuffer=_JMemBufInternal.data) \
               for (int _break = ((_JWrInternal.userData = &_JMemBufInternal), 1); _break; _break = 0)

#ifndef JStartStaticMemEx
...
// WinAPI Dynamic memory macroses
#define WJStartMemEx(JsonBuffer, isFlat)    JStart(isFlat, dyn_mem_putc, wvsprintf)WJMemOpen(JsonBuffer)
#define WJStartMemObjEx(JsonBuffer, isFlat) WJStartMemEx(JsonBuffer, isFlat)JObjBlock
#define WJStartMemArrEx(JsonBuffer, isFlat) WJStartMemEx(JsonBuffer, isFlat)JArrBlock

#define WJStartMem(JsonBuffer)              WJStartMemEx(JsonBuffer, 0)
#define WJStartMemObj(JsonBuffer)           WJStartMem(JsonBuffer)JObjBlock
#define WJStartMemArr(JsonBuffer)           WJStartMem(JsonBuffer)JArrBlock

#define WJFree GlobalFree

    #ifndef JFree
        #define JFree WJFree
        #define JPuts WJPuts
        
        #define JStartMemEx WJStartMemEx
        #define JStartMemObjEx WJStartMemObjEx
        #define JStartMemArrEx WJStartMemArrEx

        #define JStartMem WJStartMem
        #define JStartMemObj WJStartMemObj
        #define JStartMemArr WJStartMemArr
    #endif // No STDLIB
